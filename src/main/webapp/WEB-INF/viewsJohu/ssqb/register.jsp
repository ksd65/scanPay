...
function getAddrAreaList(areacode,selectName){
	$.ajax({
		url:"${ctx }/common/getAddrAreaList?oemName=${oemName}",
...
function addr_cityChange(cityVal){
	...
	function getAreaList(parentId,selectName){
		$.ajax({
			url:"${ctx }/common/getAreaList?oemName=${oemName}",
	...
	function getKBankList(){
		...
		var url = "${ctx }/common/getKBankList?oemName=${oemName}";
		var data = {};
		$.ajax({
			url:url,
			data:data,
	...
	function getBankList(bankId,selectName,settleType){
		...
		var data = {};
		if(selectName == "bankId"){
			url = "${ctx }/common/getBankList?oemName=${oemName}";
		}else if(selectName == "subId"){
			url = "${ctx }/common/getSubBankList?oemName=${oemName}";
...
function ajaxFileUpload (uploadUrl,fileInputID){ 
	...
	function ajaxSubmitForm(){
		...
		$("#registerForm").ajaxSubmit({  
            url : "${ctx }/registerMem?oemName=${oemName}",  
            type : "post",  
            dataType : 'json',  
            success : function(data) {
            	isSubmitAble = true;
            	if(data.returnCode=="0000"){//请求成功
					window.location.href = "${ctx}/registerSuccess?oemName=${oemName}";
				}else{
					alert(data.returnMsg);
				} 
            },  
	...
	function searchBank(){
		...
		var city = $.trim($("select[name='city']").val());
		$.ajax({
			url:"${ctx }/common/searchSubBank?oemName=${oemName}",
