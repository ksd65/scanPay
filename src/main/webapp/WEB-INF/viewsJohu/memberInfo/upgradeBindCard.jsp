...
<script>
	...
	function txcg(isShow,callBackUrl){
		if (isShow) {
			$("#txcg").show();
		}else{
			$("#txcg").hide();
			window.location.href = "${ctx}" + callBackUrl;
		}
	}
		...
		$("#submitForm").ajaxSubmit({
            ...
            success : function(data) {  
            	if(data.returnCode=="0000"){//请求成功
					...
					$('#err').html("原因:"+data.returnMsg);
					txsb(true);
					$('#zsbut').attr("disabled",false); 
				} 
            },  
            error : function(data) {  
            	alert("请求出错"); 
...
<form id="submitForm" action="#">
	...
	<input type="button" id="zsbut" class="zcimsbut" value="确认绑定" onclick="submitForm();">
</form>
</div>
...
<div id="txcg" class="txbfkd" style="display:none;">
	<div class="txbfkdcon">
		...
		<span>信用卡绑定成功</span>
		<p>D+0提现功能已开启<br/>感谢您的支持</p>
		<a class="cgx" href="javascript:;" onclick="txcg(false,'/memberInfo/showChannel')"></a>
	</div>
</div>
...
<div id="txsb" class="txbfkd" style="display:none;">
	<div class="txbfkdcon">
		...
		<span>绑定失败！</span>
		<p>暂不支持绑定借记卡<br/>请绑定信用卡</p>
		<a class="sbx" href="javascript:;" onclick="txsb(false)"></a>
	</div>
</div>
