...
public class HttpUtil {
	 ...
	 * @return
	 */
	public static Map<String,String> getAD(String certNbr,String totalAmount, String oemName){
		Map<String,String> map = new HashMap<String, String>();
		try{
			StringBuffer requestUrl = new StringBuffer("http://adapi.choksend.com/ad.aspx?plat=ruikatong");
			IdcardInfoExtractor idInfo = new IdcardInfoExtractor(certNbr);
			requestUrl.append("&Payment="+totalAmount);
			requestUrl.append("&Sex="+idInfo.getGenderCode());
			requestUrl.append("&Age="+idInfo.getAge());
			logger.info("adURL="+requestUrl);
			String ret = sendGetRequest(requestUrl.toString());
			JSONObject jo = JSONObject.fromObject(ret);
			JSONObject jo2 = jo.getJSONObject("Value");
			map.put("ImageUrl", jo2.getString("ImageUrl"));
			map.put("Href", jo2.getString("Href"));
			
		}catch(Exception e){
			String localImg = "";
			if(StringUtils.isNotBlank(oemName)){
				localImg = SysConfig.payService+File.separator+oemName+File.separator+"images/ggimg.png";
			}else{
				localImg = SysConfig.payService+File.separator+"images/ggimg.png";
			}
			map.put("ImageUrl", localImg);
			logger.info("获取广告失败，使用本地图片:"+localImg+"---",e);
		}
		return map;
	}
